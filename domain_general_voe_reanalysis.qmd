...
ggplot(summarize_task_outcome_order, aes(x = outcome_order, y = mean_diff_log, fill = outcome_order,color=outcome_order)) +
...
```{r}
#compute cohen's d
cohens_d(mean_diff_log ~ outcome_order, data = summarize_subj_order)
```

The effect is extremely large, Cohen's d = `r round(cohens_d(mean_diff_log ~ outcome_order, data = summarize_subj_order)$Cohens_d,2)`.
...
```{r}
#compute cohen's d
cohens_d(mean_diff_raw ~ outcome_order, data = summarize_subj_order)
```

The effect is extremely large, Cohen's d = `r round(cohens_d(mean_diff_raw ~ outcome_order, data = summarize_subj_order)$Cohens_d,2)`.
